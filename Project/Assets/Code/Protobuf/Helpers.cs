...
namespace ProtoBuf
{
    ...
    internal sealed class Helpers
    {
...
#if WINRT
#else
        static MemberInfo[] s_fields = new MemberInfo[24];

        internal static MemberInfo[] Apply(int size)
        {
            if (s_fields.Length < size)
            {
                s_fields = new MemberInfo[size];
            }
            else if (size < s_fields.Length)
            {
                s_fields[size] = null;
            }
        ...
        internal static MemberInfo[] GetInstanceFieldsAndProperties(Type type, bool publicOnly)
        {
            ...
            int size = fields.Length + props.Length;
            MemberInfo[] members = Apply(size);
